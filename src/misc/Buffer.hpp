...
public:
  ...
  struct allocator
  {
    inline ptr_type operator()(const size_t size) const { return ::operator new[](size); }
  };

  struct deleter
  {
    inline void operator()(const ptr_type data) const { ::operator delete[](data); }
  };
  ...
  inline ~Buffer() noexcept { deleter_type()(m_ptr); }
  ...
  inline Blob<_T> to_blob() {
    return { reinterpret_cast<_T *>(m_ptr), m_size / sizeof(_T) };
  }

  template <typename _T>
  inline Blob<const _T> to_blob() const {
    return { reinterpret_cast<const _T *>(m_ptr), m_size / sizeof(_T) };
  }
