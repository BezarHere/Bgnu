...
public:
  ...
  inline basic_string_cursor(const char_type *str) : basic_string_cursor(str, 0) {}

  template <size_t N>
  inline basic_string_cursor(const char_type (&str)[N]) : basic_string_cursor(str, N) {}
  inline basic_string_cursor(const string_type &str)
      : basic_string_cursor(str.c_str(), str.length()) {}
  ~basic_string_cursor();
  ...
  inline size_t size() const { return m_ref->len - m_index; }

  // is there a valid string after the cursor
  inline bool empty() const { return m_ref == nullptr || m_ref->len == 0 || m_index >= m_ref->len; }

  // the total string held (even behind the cursor's position)
  inline size_t total_size() const { return m_ref->len; }

  // same as position()
  inline size_t index() const noexcept { return m_index; }

  // same as index()
  inline size_t position() const noexcept { return m_index; }
...
template <typename _T>
inline basic_string_cursor<_T> &basic_string_cursor<_T>::operator=(
    const basic_string_cursor &copy) {
...
template <typename _T>
inline basic_string_cursor<_T> &basic_string_cursor<_T>::operator=(
    basic_string_cursor &&move) noexcept {
  _decref();
