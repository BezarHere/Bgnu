...
public:
  ...
  // "<current context>::<sub-reader data name (i.e name parameter)>" (ex: foo.branch_reader("bar")
  // -> foo::bar)
...
inline const FieldVar &FieldDataReader::try_get_value(const FieldVar::String &name,
  ...
  {
    _error("Expecting a value of type %s named '%s'", FieldVar::get_name_for_type(FType),
           name.c_str());
  ...
  {
    _error("Expecting type '%s' for '%s', got type '%s'", FieldVar::get_name_for_type(FType),
           name.c_str(), FieldVar::get_name_for_type(var.get_type()));
    return default_value;
  }
...
template <FieldVarType ElementFType>
inline const FieldVar &FieldDataReader::try_get_array(const FieldVar::String &name,
                                                      const FieldVar &default_value) const {
  ...
  for (size_t i = 0; i < array_var.get_array().size(); ++i)
  {
    ...
    {
      _error("The array '%s' should only contain %s types, but element at %llu is a/an %s type",
             name.c_str(), FieldVar::get_name_for_type(ElementFType), i,
