...
public:
  ...
  inline const FieldVar &try_get_value(
      const FieldVar::String &name,
  ...
  inline const FieldVar &try_get_array(
      const FieldVar::String &name,
      const FieldVar &default_value = DefaultArray) const;

  // will throw an exception if the reader can't be created
  // branching will fail if the value at 'name' isn't a dict (something else or
  // does not exist) also, not specifying the new_context parameter will result
  // in a reader with context equal to
  // "<current context>::<sub-reader data name (i.e name parameter)>" (ex:
  // foo.branch_reader("bar")
  // -> foo::bar)
  FieldDataReader branch_reader(const FieldVar::String &name,
...
private:
  ...
  inline void _error(const char *format, _Args &&...args) const {
    ...
    Logger::error(new_format.c_str(), std::forward<_Args>(args)...);
  }
};

template <FieldVarType FType>
inline const FieldVar &FieldDataReader::try_get_value(
    const FieldVar::String &name,
    const FieldVar &default_value) const {
  if (!_data.contains(name))
  {
    _error("Expecting a value of type %s named '%s', returning default",
  ...
  {
    _error("Expecting type '%s' for '%s', got type '%s'",
           FieldVar::get_name_for_type(FType),
           name.c_str(),
...
template <FieldVarType ElementFType>
inline const FieldVar &FieldDataReader::try_get_array(
    const FieldVar::String &name,
    const FieldVar &default_value) const {
  ...
  for (size_t i = 0; i < array_var.get_array().size(); ++i)
  {
    ...
    {
      _error(
          "The array '%s' should only contain %s types, but element at %llu is "
          "a/an %s type",
          name.c_str(),
