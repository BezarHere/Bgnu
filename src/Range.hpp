...
struct TRange
{
  static_assert(std::is_arithmetic_v<_T> || std::is_pointer_v<_T>,
                "only arithmetic or pointer ranges are allowed");

  inline constexpr TRange() = default;

  inline constexpr TRange(_T pstart, _T pend) : begin{ pstart }, end{ pend } {}

  inline constexpr _T length() const { return end - begin; }

  inline constexpr bool valid() const { return end > begin; }

  inline constexpr bool contains(const _T value) const { return value < end && value >= begin; }

  // returns a copy expanded by `value`.
  // expansion size is `2 * value` (e.g. `{1, 5}.expanded(2)` == `{-1, 7}`)
  inline constexpr TRange expended(const _T value) const { return { begin - value, end + value }; }

  // returns a copy shifted by `value` (e.g. `{2, 7}.shifted(-3)` == `{-1, 4}`)
  inline constexpr TRange shifted(const _T value) const { return { begin + value, end + value }; }
