...
bool BuildCache::too_out_dated_with(const BuildCache &cache) const {
  const auto expiration_age_seconds = Settings::Get("build_expiration_age_sec", DefaultOutdatedCacheTimeSec).get_int();
  const auto last_build_age = std::chrono::microseconds(cache.build_time - this->build_time);
  const auto last_build_age_sec_ticks = std::chrono::duration_cast<std::chrono::seconds>(last_build_age).count();
...
inline std::string ParseToHex(hash_t hash) {
  char buffer[32] = {0};
  snprintf(
    buffer, std::size(buffer), "%llX", hash
  );
  return {buffer};
}
